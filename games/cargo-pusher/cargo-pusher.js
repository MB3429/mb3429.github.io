...
function startScreen() {
  ...
  context.fillText('Cargo Pusher', width/2, height/2-90);

  iconSize = [width/2-150, height/2+10, 300, 60];
  ...
  context.fillText('PC', width/2, height/2+63);

  iconSize = [width/2-150, height/2+90, 300, 60];
  if (regionContains(clickX, clickY, ...iconSize)) {
    transition = {
      frame: 25,
      endScreen: 'levels'
  ...
  context.strokeRect(...iconSize);
  context.fillText('Mobile', width/2, height/2+143);
  
  context.drawImage(spriteList[6],0,0,32,32,width/2,height/2-72,64,64);
  context.drawImage(spriteList[1],width/2+64,height/2-72,64,64)
  context.drawImage(spriteList[8],0,0,32,32,width/2+64,height/2-72,64,64);
...
function inLevelScreen() {
  ...
  if (mobileMode) printMovement(lvlWidth, lvlHeight, 32);
...
canvas.addEventListener('mousemove', Event => {
  ...
  if (!mobileMode && !transition && !transitionIn &&
...
canvas.addEventListener('keydown', Event => {
  if (transition || transitionIn || mobileMode) return;
